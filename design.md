...
- `accountId` は **明示指定を必須** とする（暗黙解決に依存しない）
- CDK 実行時は `CDK_DEFAULT_ACCOUNT` が未設定なら、デプロイ前（最低でも `synth` 前）に失敗させる
- スタック直生成時も `env.account` 未指定・空文字をエラーとして扱い、規則違反を早期検知する
- これにより、命名規則 `{accountId}` の検証と実行時入力の整合性を常に保つ
